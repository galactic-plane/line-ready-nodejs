...
process.env['Path'] =
  process.env['Path'] + ';' + process.env['ChromeWebDriver'];
  ...
  it('Assert page title', async () => {
    ...
    while (true) {
      try {
        ...
        await driver.wait(
          until.titleIs('Line Ready - Fishing information and planning'),
          2000
        );
        break;
      } catch (err) {
        ...
        if (currentTimestamp > endTimestamp) {
          console.log(
            '##vso[task.logissue type=error;]Failed with error ' + err
          );
